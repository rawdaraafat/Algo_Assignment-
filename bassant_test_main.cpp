...
int main() {
    ...
    cout << table.search(222) << endl; // Kara

    ///test string decoding (PartB3)
    InventorySystem inv;

    cout << inv.countStringPossibilities("uu") << endl;     // 2
    cout << inv.countStringPossibilities("uunn") << endl;   // 4
    cout << inv.countStringPossibilities("unnuunn") << endl; // 8
    cout << inv.countStringPossibilities("ununnuunnu") << endl; // 8
}
